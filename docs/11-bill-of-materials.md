...
| Digital airspeed sensor — **Matek ASPD-4525** (settled pick, ~€35) | VTOL transition safety — ground speed ≠ airspeed in wind, so ArduPilot's quadplane hover↔cruise transition is more reliable with real airspeed than a GPS-groundspeed/schedule fallback. Chosen over a DLVR-based sensor (~€100+, no pitot-cover ritual needed) — not worth ~3x the price when `ARSPD_SKIP_CAL=1` (confirmed real — also documented directly in the CoreWing FC's own parameter table, "disable indoor calibration") plus `ARSPD_AUTOCAL` gets the same no-cover-ritual convenience for free on the cheaper sensor. **MS4525DO chip**, I2C, 4-6V, 1 PSI range (~360-380 km/h max airspeed — comfortably covers the ~80-120 km/h cruise target), ±0.25% span accuracy, 3.5g, includes pitot tube + cable + tubing. Slots into hardware already reserved for it: FC's single I2C port (shared with GPS compass) — [Flight Controller](03-flight-controller.md#f405-fc-specifications). | 🛒 later — post-validation |

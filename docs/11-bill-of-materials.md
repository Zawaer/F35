...
| GPS module — **HGLRC M100-5883** (✅ FINAL — decision closed, don't re-litigate) | ArduPilot VTOL position hold (EKF) + heading (built-in **QMC5883P** compass, confirmed via manufacturer comparison table — no separate compass module needed). M10 chipset, **GPS+Galileo+BDS only — no GLONASS** (that's the pricier B101-chip M100-PRO instead), 72 channels, 10Hz update, 21×21×8.02mm, 7.5g, **~€28 on AliExpress** (price quoted elsewhere online ranged €12-21 — actual checkout price is what matters, confirmed ~€28). Wiring: UART → **F405 UART5** (the one genuinely spare UART — see [Flight Controller port table](03-flight-controller.md#port-assignments); do *not* use UART3, that's already the WeAct avionics MAVLink link), compass SDA/SCL → the F405's single I2C port (already earmarked for compass/airspeed). Also reusable as-is on a future long-range fixed-wing build — nothing F-35B-specific about a GPS+compass module. **Pre-flight validation (before first hover):** run ArduPilot's built-in **CompassMot** calibration (props/fan blades off, aircraft restrained) to measure and correct for magnetic interference from the dual 70mm EDFs' high current draw; twist the main power wires and keep the compass I2C leads away from ESC phase lines; mount the module as far from power wiring as the airframe allows — distance matters far more than shielding for a magnetic (not electric) interference source. **Why this over the alternatives (M100-Pro/B101, BZGNSS P18 Pro+IST8310, Beitian BE-180):** proven u-blox M10 + ArduPilot compatibility outweighs marginal constellation/compass-noise gains from less-proven or pricier options — see conversation history 12 Jul 2026 for the full comparison if this ever needs revisiting (e.g. if real bench-test satellite count/PDOP once purchased turns out to be actually inadequate, not just theoretically worse). | 🛒 later — post-validation |
| Downward rangefinder — **TFmini Plus** (settled on, over TF-Luna/VL53L1X/TF02-Pro) | Low-altitude hover hold outdoors. Chosen over TF-Luna specifically because a real spec comparison shows TF-Luna has **low ambient-light immunity** (contradicting an earlier claim that it's "sunlight-immune") vs TFmini Plus's **high immunity (70 Klux)** — the exact property that matters most here. Bonus: TFmini Plus is **IP65 sealed** (bare-PCB TF-Luna isn't) — relevant since this flies low over grass/dust/moisture. 12m indoor/7m outdoor range, 10cm blind zone (tighter than TF-Luna's 20cm, better for landing flare), only +7g over TF-Luna (negligible vs ~3445g AUW). TF02-Pro considered and dropped — 40m range and 50g weight are both overkill for a sensor that only matters in the 0-3m hover/landing band. Owned VL53L0X (bench-tested ±2-3mm indoor, [Sensors](07-sensors-monitoring.md#other-monitoring-ideas)) stays as-is for bench work / possible landing-flare-only use. ⚠️ "Centimeter-precision hover" claims for this class of sensor/stack are still optimistic marketing framing, not a guarantee for a DIY ArduPilot setup — expect degraded real-world performance vs datasheet numbers regardless of which sensor is picked. | 🛒 later — post-validation |
| PMW3901 optical flow sensor | Horizontal drift/velocity sensing — pairs with the downward rangefinder (needs its altitude reading to convert pixel-shift to real velocity); ArduPilot has native `OPTFLOW` support. Complements, doesn't replace, the rangefinder — covers a different axis (horizontal drift) that altitude sensors don't touch. | 🛒 later — post-validation |
| Digital airspeed sensor — **Matek ASPD-4525** (settled pick, ~€35) | VTOL transition safety — ground speed ≠ airspeed in wind, so ArduPilot's quadplane hover↔cruise transition is more reliable with real airspeed than a GPS-groundspeed/schedule fallback. Chosen over a DLVR-based sensor (~€100+, no pitot-cover ritual needed) — not worth ~3x the price just to skip a 2-second startup step; `ARSPD_AUTOCAL` (confirmed real ArduPilot param) plus manually covering the pitot tube at power-on gets most of the same benefit for free. **MS4525DO chip**, I2C, 4-6V, 1 PSI range (~360-380 km/h max airspeed — comfortably covers the ~80-120 km/h cruise target), ±0.25% span accuracy, 3.5g, includes pitot tube + cable + tubing. Slots into hardware already reserved for it: FC's single I2C port (shared with GPS compass) — [Flight Controller](03-flight-controller.md#f405-fc-specifications). ⚠️ `ARSPD_SKIP_CAL` (a claimed parameter to skip the startup pitot-cover step) is **not verified** — check Mission Planner's actual parameter list for the ArduPilot version in use before relying on it; if it doesn't exist, just cover the pitot tube by hand at power-on as MS4525DO has always required. | 🛒 later — post-validation |
